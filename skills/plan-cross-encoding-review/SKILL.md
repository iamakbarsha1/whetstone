...
- **Redundant-value agreement.** Any two hardcoded values related by a pure
  ...
  Reused magic pairs multiply the blast radius across tasks — a fixture
  timestamp labelled with a human date that is actually a day off, copied
  into three tasks, ships the bug three times.
- **Prose-vs-code agreement.** When a task gives both a prose description of
  ...
  on data-loss or security edges.
- **Guard shape and ordering.** When a plan specifies a guard over
  ...
  failure is a safe retry rather than corruption.
